...
DATASET_FILE = _DATA_DIR / "DataSetTeensyv9_ULTRA_CLEAN.csv"
N_TRIALS = 80
RANDOM_SEED = 44
INPUT_SIZE = 8192
...
def export_to_cpp(pipeline, params, categories):
    ...
    cpp_filename = _ARTIFACT_DIR / f"model_teensy_{timestamp}.h"
...
def run_tuner():
    ...
    results_file = _ARTIFACT_DIR / f"best_results_{timestamp}.json"
    with open(results_file, "w") as f:
        json.dump(results, f, indent=2)

    model_file = _ARTIFACT_DIR / f"final_model_{timestamp}.joblib"
    joblib.dump(final_pipeline, model_file)

    trials_file = _ARTIFACT_DIR / f"trial_summary_{timestamp}.csv"
    ...
    try:
        ...
        importance_fig.write_html(_ARTIFACT_DIR / f"param_importance_{timestamp}.html")

        # Historique d'optimisation
        history_fig = plot_optimization_history(study)
        history_fig.write_html(_ARTIFACT_DIR / f"optimization_history_{timestamp}.html")
    ...
    logger.info(f"   C++ Export: {cpp_file}")
